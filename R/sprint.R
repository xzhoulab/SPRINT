...
#
######################################################################
#' Each SPRINT object has a number of slots which store information. Key slots to access
#' are listed below.
#'
...
#' Create the SPRINT object with filtering step
...
#' @param prectage The prectage of cells that are expressed for analysis
#' @return Returns SPRINT object with filtered gene expression matrix
#' 
#' @export
CreateSPRINTObject <- function(counts, location, project = "SPRINT", prectage = 0.1, min_total_counts = 10){
  ...
  counts <- as(counts, "dgCMatrix")# store as sparse matrix
  ...
  # filtering out genes
  if(prectage > 0){
    gene_use <- which( rowSums(object.counts > 0) >= floor(prectage*ncol(object.counts)) )
