...
## Summarized the multiple p-values via ACAT method
...
CombinePValues <- function(pvalues){
  ...
  combinedp <- 1.0/2.0 - atan(Cbar)/pi
  return(combinedp)
}# end function



## Normalize the gene expression count matrix
#' @param counts Gene expression count matrix
#' @param method Normalization method, i.e. vst, uniform
#' @export
normalize_counts <- function(counts, method="vst"){
  # # select different normalization method
  # if(method=="vst"){
    # sv <- 1
  ...
  # }else if(method=="uniform"){
    # norm_counts <-  (counts - min(counts))/(max(counts) - min(counts))
  # }# end fi
